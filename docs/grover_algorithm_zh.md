...
### 11.6 常见疑问：AES oracle 中 `f(K)=1` 不就已经找到密钥了吗

这句话“逻辑上对”，但“算法上还没有解决搜索问题”。

对于 AES 已知明文/密文攻击，判定函数可以写成：
...
如果某次你已经拿到了一个具体候选密钥 `K`，并且计算出 `f(K)=1`，当然就找到密钥了。经典暴力破解正是这么做的：
...
难点不是“如何验证一个给定 K 是否正确”，而是：

```text
不知道哪个 K 会让 f(K)=1。
```

也就是说，oracle 是一个检查器，不是答案打印机。

### 11.7 经典暴力破解也有判定准则，为什么不能同样加速

经典暴力破解和 Grover 使用的是同一个判定准则：
...
| 方式 | 怎么调用判定函数 | 结果 |
| --- | --- | --- |
| 经典暴力破解 | 逐个候选 `K` 调用 `f(K)` | 平均约 `2^(k-1)` 次，量级 `O(2^k)` |
| Grover | 在密钥叠加态上可逆调用 `f(K)`，只做相位标记 | 约 `O(2^(k/2))` 次 oracle 调用后，正确 key 振幅变大 |

Grover 的优势不是判定更聪明，而是：

```text
相位标记 + diffuser 让正确 key 的振幅逐轮变大。
```

```mermaid
flowchart LR
    A["经典\n一次检查一个 K"] --> B["f(K)=1 时停止"]
    B --> C["但通常要查 O(2^k) 个候选"]

    D["Grover\n所有 K 的叠加态"] --> E["oracle 给正确 K 加负相位"]
    E --> F["diffuser 放大正确 K 振幅"]
    F --> G["约 O(2^(k/2)) 轮后测量"]
```

### 11.8 量子 oracle 不是测量 `f(K)`
...
然后直接测量第二个寄存器，单目标密钥搜索中测到 `1` 的概率是：

```text
1 / 2^k
```

这和随机猜一个 key 没有本质区别，而且测量会破坏叠加态。

Grover 使用的 phase oracle 更像：
...
这个负号本身不能直接读出来。它要经过 diffuser 才会变成更大的测量概率。

### 11.9 AES 的 Grover oracle 大致如何构造
...
1. AES 必须实现成可逆量子电路，因为量子演化要保持可逆。
2. 比较和相位标记后要“反算”AES，把辅助寄存器恢复干净，否则垃圾信息会和 key 纠缠，破坏后续 diffuser 的效果。

因此真实 Grover-AES 攻击的 oracle 很昂贵，并不是“免费检查一次 AES”。

### 11.10 为什么不能按 AES 输出的某一位逐步推出 key

另一个常见误解是：既然 oracle 能判断 `AES_K(P) == C`，那能不能先判断第一位、第二位，然后逐步缩小 key？

问题是 AES 被设计成强扩散、强混淆。一个 key bit 的改变会让输出密文整体看起来像随机变化。检查某一位：

```text
AES_K(P)[0] == C[0]
```

只能筛掉大约一半候选，但不会告诉你“密钥哪一位该怎么改”。对理想分组密码来说，映射：

```text
K -> AES_K(P)
```

应当近似随机。没有可利用的方向、梯度或逐位修正规则。

所以经典算法即使有同样判定准则，仍然无法从中获得 Shor 式结构性加速；Grover 能做的是对无结构搜索进行平方加速。
...
经典暴力破解逐个 K 查询 f(K)；
Grover 用相位 oracle 标记正确 K，再用 diffuser 放大它。

直接测量 f(K) 的成功概率仍是 1/2^k；
相位标记本身也不能读出答案。

AES oracle 的判定准则不神奇；
量子加速来自振幅放大，不来自更聪明的判定。
```
...
11. AES oracle 中 `f(K)=1` 为什么不等于算法已经知道了正确 `K`？
12. 直接测量 `|f(K)⟩` 为什么不能带来 Grover 加速？
13. Grover-AES oracle 为什么需要反算 AES？
14. 为什么检查 AES 输出的某一位不能像解方程一样逐步推出 key？
...
11. `f(K)=1` 只说明某个给定候选 `K` 正确；搜索难点是你不知道哪个 `K` 会让 `f(K)=1`。
12. 单目标搜索中测到 `f(K)=1` 的概率仍是 `1/2^k`，而且测量会破坏叠加态；Grover 需要相位标记和 diffuser。
13. 反算可以清理辅助寄存器，避免垃圾信息残留并破坏后续振幅放大。
14. AES 设计成强扩散、强混淆；单个输出位匹配只筛掉部分候选，不提供密钥位的修正方向。
