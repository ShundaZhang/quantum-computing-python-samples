# 量子力学趣味专题：双缝、隧穿、EPR、贝尔不等式与薛定谔的猫

这篇文档不从代码开始，而从几个著名思想实验和现象开始：双缝干涉、量子隧穿、EPR 佯谬、贝尔不等式、薛定谔的猫，以及“量子纠缠到底是不是超距作用、能不能超光速传递信息、会不会突破因果律”这些经常让人兴奋也经常让人误解的问题。

如果你遇到缩略语，可以先查 [常用词汇表](glossary_zh.md)，尤其是 [双缝实验](glossary_zh.md#double-slit)、[量子隧穿](glossary_zh.md#quantum-tunneling)、[波函数](glossary_zh.md#wavefunction)、[纠缠](glossary_zh.md#entanglement)、[Bell 态](glossary_zh.md#bell-state)、[EPR](glossary_zh.md#epr)、[贝尔不等式](glossary_zh.md#bell-inequality)、[无信号定理](glossary_zh.md#no-signaling)、[退相干](glossary_zh.md#decoherence)。
...
| 一个粒子应该走一条确定路径 | 双缝实验显示单粒子也会积累出干涉图样 |
| 能量不够就绝对过不了墙 | 量子隧穿允许波函数穿透势垒并有概率出现在另一侧 |
...
## 趣味实验 A：双缝干涉，粒子如何像波一样“自己和自己干涉”

双缝干涉是理解量子力学最经典的实验之一。

想象我们向一块带有两条狭缝的挡板发射粒子，后方有一块探测屏。
...
但量子实验告诉我们：如果不测量粒子到底过了哪条缝，即使一次只发射一个粒子，长期累积也会出现干涉条纹。
...
关键点是：量子力学不是先给粒子分配一条确定路径，再把路径概率相加；它先把两条路径的 **概率幅** 相加，再取模平方得到概率。

```text
经典概率：P = P_left + P_right
量子概率：P = |amplitude_left + amplitude_right|^2
```

因为振幅可以有相位，两个路径的振幅会相加增强，也会相消抵消。
...
最令人惊讶的是：一次只发一个粒子时，每次屏幕上只出现一个点；但点积累多了，整体仍形成干涉条纹。
...
如果我们放一个探测器，测量粒子到底经过左缝还是右缝，干涉图样会消失，屏幕更像两个经典亮斑。
...
flowchart LR
    ...
    D["测量路径"] --> E["获得 which-path 信息"]
    E --> F["相干性被破坏"]
    F --> G["干涉消失"]
```

这不是因为探测器“粗暴撞坏了粒子”这么简单。更深的原因是：一旦路径信息被记录到环境或仪器中，两条路径就不再能像同一个整体振幅那样相干叠加。

这和后面薛定谔的猫、退相干、测量问题会连起来。

## 趣味现象 B：量子隧穿，为什么粒子能穿过经典上过不去的势垒

量子隧穿说的是：一个粒子的能量低于势垒高度时，经典力学认为它过不去；量子力学却允许它有一定概率出现在势垒另一侧。
...
注意这不是说粒子“借能量作弊”然后再还回去。更稳妥的说法是：

```text
能量本征态在势垒区域仍有满足薛定谔方程的衰减解；
粒子的位置测量有一定概率出现在势垒另一侧。
```
...
它和双缝实验一起提醒我们：微观粒子不是经典小球，波函数的空间分布、相位和边界条件会直接影响可观测概率。
...
## 13. 双缝、隧穿、EPR、Bell、猫之间的关系

这些故事常被分开讲，其实它们在问同一个大问题：
...
| 主题 | 核心问题 |
| --- | --- |
| 双缝干涉 | 为什么路径概率幅会相干相加并形成条纹 |
| 量子隧穿 | 为什么经典过不去的势垒仍有非零通过概率 |
...
11. 双缝实验中，为什么“不看路径”时会出现干涉条纹？
12. 一旦获得 which-path 信息，为什么干涉通常消失？
13. 量子隧穿是不是粒子临时借了能量？
14. 隧穿概率和势垒宽度、高度大致有什么关系？
...
11. 因为两条路径的概率幅保持相干，先相加再取模平方，导致相长和相消干涉。
12. 因为路径信息被仪器或环境记录后，两条路径不再作为同一个相干振幅叠加。
13. 不是。更准确地说，波函数在势垒区域有衰减但非零的解，因此另一侧有非零探测概率。
14. 势垒越宽、越高，隧穿概率通常越小；粒子能量越接近势垒高度，隧穿概率越大。

## 21. 核心结论

1. **双缝干涉揭示概率幅相加**：量子概率来自振幅相加后的模平方，不是普通路径概率相加。
2. **量子隧穿揭示波函数延伸**：经典禁区里波函数不一定为零，因此有非零穿透概率。
3. **EPR 佯谬提出问题**：量子力学是否完备，远距离纠缠如何理解。
4. **贝尔不等式给出实验判据**：局域隐变量模型必须满足 `|S| <= 2`，量子纠缠可超过它。
5. **实验支持量子非经典关联**：纠缠关联不能被朴素局域隐藏答案表解释。
6. **纠缠不能超光速传递有效信息**：单边结果随机，必须事后经典通信比较才看到关联。
7. **因果律没有被突破**：标准量子理论保留无信号结构。
8. **薛定谔的猫聚焦测量问题**：它提醒我们解释微观叠加到宏观确定性的过渡并不简单。
